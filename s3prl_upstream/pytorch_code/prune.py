r"""
Pruning methods
This file is licensed by the "LICENSE" file in the same directory
"""
import numbers
...
class BasePruningMethod(ABC):
    ...
    def __call__(
        ...
        Args:
            ...
            inputs: not used.
        """
        module._buffers[self._tensor_name] = self.apply_mask(module)

    @abstractmethod
    def compute_mask(
        self,
        t:torch.Tensor,
        default_mask:torch.BoolTensor
        ) -> torch.BoolTensor:
    ...
    def apply_mask(self, module: Module) -> torch.Tensor:
        ...
        mask = mask.bool()
        pruned_tensor = orig.masked_fill(~mask, 0)
    ...
    def apply(
        ...
        name:str,
        *args,
        importance_scores: Union[Dict[Tuple[Module, str], torch.Tensor],None]=None,
        **kwargs
        ):
        ...
        if not isinstance(method, PruningContainer):
            ...
            default_mask = torch.ones_like(orig, dtype=torch.bool)  # temp
        ...
        try:
            ...
            module.register_buffer(name, method.apply_mask(module), persistent=False)
    ...
    def prune(
        self,
        t:torch.Tensor,
        default_mask:Union[torch.BoolTensor, None] =None, importance_scores: Union[Dict[Tuple[Module, str], torch.Tensor],None]=None,
        inplace:bool=False,
        ):
        ...
        if importance_scores is not None:
            ...
            importance_scores = t
        default_mask = default_mask if default_mask is not None else torch.ones_like(t, dtype=torch.bool)
        mask = self.compute_mask(importance_scores, default_mask=default_mask)
        if inplace:
    ...
    def remove(
        ...
        if hasattr(module, self._tensor_name):
            del module._buffers[self._tensor_name]
...
class PruningContainer(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask) -> torch.BoolTensor:
        ...
        * for 'global', the mask will be computed across all entries.
        ...
        def _combine_masks(
            ...
            """
            mask = mask.bool() # start off from existing mask
            ...
            # entirely pruned
                ...
                keep_channel = mask.sum(dim=[d for d in range(n_dims) if d != dim]).bool()
...
class L1Unstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        if nparams_toprune:  # k=0 not supported by torch.kthvalue
            # largest=True --> top k; largest=False --> bottom k
            # Prune the smallest k
            topk = t.abs().view(-1).topk(k=nparams_toprune, largest=False)
            # topk will have .indices and .values
            mask.view(-1)[topk.indices] = False
...
class CustomFromMask(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        mask = default_mask.bool() & self.mask.bool()
        return mask
...
def global_unstructured(
    parameters,# : Iterable[Tuple[torch.nn.Module, str]], remove since python3.8 doesn't support
    ...
    **kwargs
    ...
    default_mask = torch.nn.utils.parameters_to_vector(
        [
            getattr(module, name + "_mask", torch.ones_like(getattr(module, name), dtype=torch.bool))
            for (module, name) in parameters
        ]
    )
...
def _validate_structured_pruning(t: torch.Tensor) -> None:
